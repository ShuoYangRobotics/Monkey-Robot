function [new_x_list,new_u_list,J,lamb, sim_new_trajectory, converge] = monkey_iLQR_iteration(x0, xT, new_u_list, new_x_list,  dt, step, Q, R, Qf, lamb, sim_new_trajectory)
    lamb_factor = 2.5;
    lamb_max = 10000;
    % store fx fu lx lu
    ...
    if sim_new_trajectory == 1
        ...
        [sim_x_list, old_J] = monkey_complete_run(x0, xT, new_u_list, dt, step, Q, R, Qf);
...
%         V_xx = Q_xx - fb_K(:,:,i)'*Q_uu*fb_K(:,:,i);
    ...
    for i = 1:step
        ...
        tmp_state = update_x;
        update_x = update_x + dt*monkey_dyn_func(tmp_state, [0;update_u_list(:,i)]);
    end
    
    %evaluate the update trajectory
    [update_x_list, new_J] = monkey_complete_run(x0, xT, update_u_list, dt, step, Q, R, Qf);
