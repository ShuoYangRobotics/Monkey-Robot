function [x, u, L, Vx, Vxx, cost, trace, stop] = UDP(DYNCST, x0, u0, Op, dynamics, dt, scale)
...
%
%
% This code is adapted from the iLQG algorithm of Tassa, Mansard, and Todorov:
% BIBTeX:
%
% @INPROCEEDINGS{
...
%  
%  3) derivatives:
%   [~,~,cx,cu,cxx,cxu,cuu] = DYNCST(x,u,I) computes the cost function
...
for iter = 1:Op.maxIter
    ...
    %====== STEP 1: differentiate cost function along new trajectory
    if flgChange
        t_diff = tic;
        [~,~,cx,cu,cxx,cxu,cuu] = DYNCST(x, [u nan(m,1)], 1:N+1);
    ...
    while ~backPassDone
        ...
        t_back   = tic;
        [diverge, Vx, Vxx, l, L, dV] = back_pass(cx,cu,cxx,cxu,cuu,x,u,lambda,Op.regType,Op.lims,dynamics,dt,scale);
    ...
    stop = graphics(Op.plot,x,u,cost,L,Vx,Vxx,trace(1:iter),0);
end
...
%     timing   = [diff_t back_t fwd_t total_t-diff_t-back_t-fwd_t];
    graphics(Op.plot,x,u,cost,L,Vx,Vxx,trace,2); % draw legend
else
...
function [diverge, Vx, Vxx, k, K, dV] = back_pass(cx,cu,cxx,cxu,cuu,x,u,lambda,regType,lims,plant,dt,scale)
    ...
    for i = N-1:-1:1
        ...
        [S,d] = chol(blkdiag(Vxx(:,:,i+1)^-1, cuu(:,:,i)^-1), 'lower');
        if d
            diverge  = i;
            return;
        end
        ...
        for j = 1:(2*(n+m))
            Sig(1:n,j) = rkstep_b(Sig(:,j),n,dt,plant);
        end
        ...
        QxQu = D\df;
        Qx = QxQu(1:n) + cx(:,i); %add on one-step cost
        Qu = QxQu((n+1):end) + cu(:,i); %add on one-step cost
        ...
        H = M^-1;
        H(1:n,1:n) = H(1:n,1:n) + cxx(:,:,i); %add in one-step state cost for this timestep

        Qxx = H(1:n,1:n);
        Quu = H(n+1:end,n+1:end);
        Qux = H(n+1:end,1:n);

%         if(regType == 2)
%             Vxx_reg = Vxx(:,:,i+1) + lambda*eye(n);
%             Qux_reg = cxu(:,:,i)' + fu(:,:,i)'*Vxx_reg*fx(:,:,i); %Have to implement sigma point stuff
%         else
%             Vxx_reg = Vxx(:,:,i+1);
%             Qux_reg = Qux;
%         end
        ...
        if(regType == 1)
            QuuF = Quu + lambda*eye(m); %cuu(:,:,i)  + fu(:,:,i)'*Vxx_reg*fu(:,:,i) + lambda*eye(m);
%         else
%             QuuF = Quu; %cuu(:,:,i)  + fu(:,:,i)'*Vxx_reg*fu(:,:,i);
        end
...
function  stop = graphics(figures,x,u,cost,L,Vx,Vxx,trace,init)
    stop = 0;
    ...
    if figures ~= 0  && ( mod(mT,figures) == 0 || init == 2 )
        ...
        fig1 = findobj(0,'name','U-DDP');
        if  isempty(fig1)
            fig1 = figure();
            set(fig1,'NumberTitle','off','Name','U-DDP','KeyPressFcn',@Kpress,'user',0,'toolbar','none');
...
function x0 = rkstep_b(x1u0,Nx,dt,dynamics)
    %Backwards 4th order Runge-Kutta step from x_{k+1} to x_k
    xdot1 = dynamics(0,x1u0(1:Nx),x1u0((Nx+1):end));
    xdot2 = dynamics(0,x1u0(1:Nx)-.5*dt*xdot1,x1u0((Nx+1):end));
    xdot3 = dynamics(0,x1u0(1:Nx)-.5*dt*xdot2,x1u0((Nx+1):end));
    xdot4 = dynamics(0,x1u0(1:Nx)-dt*xdot3,x1u0((Nx+1):end));
